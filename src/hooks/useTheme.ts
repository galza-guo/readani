...
export function useTheme(theme: ThemeMode, accentColor: AccentColor): void {
  ...
  useEffect(() => {
    ...
    const values = isDark
      ? ACCENT_PALETTE[accentColor].dark
      : ACCENT_PALETTE[accentColor].light;

    root.style.setProperty("--accent", values[0]);
    root.style.setProperty("--accent-hover", values[1]);
    root.style.setProperty("--accent-strong", values[2]);
    root.style.setProperty("--accent-muted", values[3]);
    root.style.setProperty("--highlight", values[4]);
    root.style.setProperty("--highlight-strong", values[5]);
    root.style.setProperty("--sentence-active", values[6]);
  }, [accentColor, theme]);
}
